...
Only three default config fields are removed before byte comparison: tracker,
ReID path and the explicitly false ROI option. This cache replay verifies
identity collection, not neural detector parity; real ROI controls are separate.
...
def main() -> None:
    ...
    added = {name: code_hash(name) for name in sorted(ROI_INTEGRATION_ALLOWLIST)}
    ...
    for group in ("day", "night", "night_extra", "night_control"):
        ...
        for feature_path in features:
            ...
            for variant, refined in (("before", False), ("after", True)):
                ...
                assert payload["config"]["detector"].pop("roi_single_batch") is False
