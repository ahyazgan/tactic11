"""Frozen, source-separated consensus control acquisition and production replay.
...
ROOT = Path("data/tracking/bench/consensus_control_v1")
SOURCES = {
    "day": dict(match=117093, segments={60: 1800, 62: 1860},
                template="data/tracking/bench/identity_day_detections/seg_0000.json"),
    "night": dict(match=117092, segments={80: 2400, 82: 2460},
...
def freeze() -> None:
    if FREEZE.exists():
        raise ValueError("Decision already frozen")
    code = list(Path("app/tracking").rglob("*.py")) + [Path(__file__), Path("tests/test_consensus_control.py")]
    ...
    inputs = [Path(DEFAULT_MODEL), Path("data/tracking/models/rfdetr_mixed_small/checkpoint_best_total.pth"),
              ...
              Path("docs/KIMLIK-UZLASISI-KONTROL-PLANI.md"),
              Path("docs/SABIT-KAMERA-KONTROL-ETIKETLEME.md")]
    inputs += [Path("docs/measurements") / n for n in (
        "identity-consensus-development-results.json", "identity-consensus-production-parity.json",
    ...
    write_new(FREEZE, dict(created_utc=datetime.now(UTC).isoformat(),
        ...
        control_pixels_opened=False, profile="bytetrack-osnet-color-consensus-v1",
...
def replay(group: str, decision: dict) -> None:
    ...
    for segment in spec["segments"]:
        ...
        for backend in ("supervision", "consensus"):
            cfg = pipeline.PipelineConfig(**{k: v for k, v in spec["config"].items() if k != "detector"})
            cfg.detector = DetectorConfig(**spec["config"]["detector"])
            cfg.tracker_backend = backend
            ...
            with (patch.object(pipeline, "collect_observations", side_effect=collect),
                ...
                _, summary = pipeline.process_video(Path(raw["video"]), PitchCalibration.from_dict(spec["calibration"]),
                    ...
                    match_id=990904, home_team_id=217, away_team_id=213)
    ...
    verify()
    write_new(out / "manifest.json", dict(freeze_sha256=digest(FREEZE), label_seal_sha256=digest(root / "label-seal.json")))
