...
from scripts.soccertrack_v2.consensus_control import FREEZE, ROOT, digest, load, verify, write_new
...
def evaluate(group: str, decision: dict) -> dict:
    ...
    if manifest != dict(freeze_sha256=digest(FREEZE), label_seal_sha256=digest(seal_path)):
    ...
    for backend in ("supervision", "consensus"):
        ...
        for segment in segments:
            ...
            if backend == "consensus":
                def visit(value, segment=segment):
                    if isinstance(value, dict):
                        if "identity_consensus" in value:
                            boundaries.extend(dict(segment=segment, **b) for b in value["identity_consensus"]["boundaries"])
    ...
    failures = regressions(results["supervision"], results["consensus"])
    for metric in ("conflicting_track_labels", "not_person_remaining"):
        if results["consensus"]["kit"][metric] > results["supervision"]["kit"][metric]:
            failures.append("kit." + metric)
    if results["consensus"]["output_observations"] < results["supervision"]["output_observations"]:
        failures.append("output_observations")
    return dict(results=results, regressions=failures, boundaries=boundaries,
