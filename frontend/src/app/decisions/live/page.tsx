...
function demoSnapshot(minute: number): LiveDecisionResponse {
  // 80. dk berabere 1-1 senaryosu — closing/star/foul/risk tetiklensin
  const trailing = minute >= 75;
  return {
    match_id: 9300, current_minute: minute, score: "1-1",
    momentum: {
      score: -0.42, holder: "opponent", press_breaking: true,
      xg_swing_alert: false,
      alert_text: "Rakip son 10 dk'da baskıyı kurdu — pres kırılıyor",
    },
    sub_timing: {
    ...
    star_feed: {
      involvement_state: "starved", suggested_action: "ON",
      pass_share_pct: 3.4,
      tactical_advice: "Yıldız top almıyor — kanat oyuncuları içe çek, dik pas hattı yarat",
    },
    foul_pressure: {
      tactical_fouling_alert: true, our_high_foul_alert: false,
      escalation_alert: true, referee_card_pressure: "high",
      tactical_advice:
        "Rakip ritim kırma fauluyor — hızlı restart, ayağa kalk · Hakem kart eşiğinde — temaslı pres düşür",
    },
    context: {
      one_liner: trailing
        ? "ŞİMDİ: Berabere · son 15 dk → tempo: yükselt (güven: yüksek)"
        : "ŞİMDİ: Mevcut planı koru — net karar yok",
      primary: trailing ? {
        headline: "Berabere · son 15 dk → tempo: yükselt, ikame: hücumcu",
        theme_label: "oyun yönetimi",
        urgency: 0.8, confidence: 0.78, confidence_label: "yüksek",
        rationale: "80. dk, berabere — closing_strategy primary; "
          + "aynı anda: foul_pressure (hakem eşiği), star_feed (yıldız aç) → karar net",
      } : null,
      secondary: trailing ? [
