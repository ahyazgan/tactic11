...
- [x] Tahmin çekirdeği v3 — rakip-göreli güç (opp_defense) + ev sahibi avantajı; backward-compat baseline korunur  (2abb8dd)
- [x] Kalibrasyon düzeltici v2 — temperature scaling; ham olasılıkları log-loss minimize eden T ile dürüstleştirir; /admin/calibration önerir; 2093 test yeşil  (HEAD)
- [ ] Kalibrasyon T'yi predict serving path'ine bağla (ρ gibi fit→cache→apply; canlı tahminleri otomatik düzelt)
